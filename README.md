<div align="center">
  ...
  <h3 align="center">A full stack Convergs</h3>
...
Build a full stack Converg using Next.js 14+ with a redesigned look transformed from a Figma design, user interaction to community management, technical implementation, and various features, including nested deep comments, notifications, real-time-search, and more.
...
👉 **Visually Appealing Home Page**: A visually appealing home page showcasing the latest convergs for an engaging user experience.

👉 **Create Converg Page**: A dedicated page for users to create convergs, fostering community engagement

👉 **Commenting Feature**: A commenting feature to facilitate discussions within convergs.

👉 **Nested Commenting**: Commenting system with nested convergs, providing a structured conversation flow.

👉 **User Search with Pagination**: A user search feature with pagination for easy exploration and discovery of other users.

👉 **Activity Page**: Display notifications on the activity page when someone comments on a user's converg, enhancing user engagement.
...
👉 **Admin-Specific Community Convergs**: Enable admins to create convergs specifically for their community.

👉 **Community Search with Pagination**: A community search feature with pagination for exploring different communities.

👉 **Community Profiles**: Display community profiles showcasing convergs and members for a comprehensive overview.
...
```bash
git clone https://github.com/vugar-gasimov/convergs.git
cd convergs
```
...
export async function deleteCommunity(communityId: string) {
  try {
    ...
    // Delete all convergs associated with the community
    await Converg.deleteMany({ community: communityId });
...
export const sidebarLinks = [
  ...
  },
  {
    ...
    route: "/create-converg",
    label: "Create Converg",
  },
  {
...
export const profileTabs = [
  { value: "convergs", label: "Convergs", icon: "/assets/reply.svg" },
...
<details>
<summary><code>converg.actions.ts</code></summary>
...
export async function fetchPosts(pageNumber = 1, pageSize = 20) {
  ...
  // Create a query to fetch the posts that have no parent (top-level convergs) (a converg that is not a comment/reply).
  const postsQuery = Converg.find({ parentId: { $in: [null, undefined] } })
    .sort({ createdAt: "desc" })
    .skip(skipAmount)
    .limit(pageSize)
    .populate({
  ...
  // Count the total number of top-level posts (convergs) i.e., convergs that are not comments.
...
export async function createConverg({
  ...
  try {
    ...
    throw new Error(`Failed to create converg: ${error.message}`);
  }
}
...
export async function deleteConverg(id: string, path: string): Promise<void> {
  try {
    ...
    // Recursively delete child convergs and their descendants
...
export async function fetchConvergById(convergId: string) {
  ...
  try {
      ...
      .populate({
        ...
        populate: [
          ...
          },
          {
            ...
            model: Converg, // The model of the nested children (assuming it's the same "Converg" model)
            populate: {
    ...
    console.error("Error while fetching converg:", err);
    throw new Error("Unable to fetch converg");
  }
}
...
export async function addCommentToConverg(
  ...
  try {
    // Find the original converg by its ID
    ...
    // Create the new comment converg
    const commentConverg = new Converg({
      text: commentText,
      author: userId,
      parentId: convergId, // Set the parentId to the original converg's ID
    });

    // Save the comment converg to the database
    const savedCommentConverg = await commentConverg.save();

    // Add the comment converg's ID to the original converg's children array
    originalConverg.children.push(savedCommentConverg._id);

    // Save the updated original converg to the database
...
export async function fetchUserPosts(userId: string) {
  try {
    ...
    // Find all convergs authored by the user with the given userId
    ...
    console.error("Error fetching user convergs:", error);
    throw error;
  }
}
...
export async function getActivity(userId: string) {
  try {
    ...
    // Find all convergs created by the user
    const userConvergs = await Converg.find({ author: userId });

    // Collect all the child converg ids (replies) from the 'children' field of each user converg
    const childConvergIds = userConvergs.reduce((acc, userConverg) => {
      return acc.concat(userConverg.children);
    }, []);

    // Find and return the child convergs (replies) excluding the ones created by the same user
    const replies = await Converg.find({
      _id: { $in: childConvergIds },
      author: { $ne: userId }, // Exclude convergs authored by the same user
    }).populate({
      path: "author",
      model: User,
